...
long CapTouch::touchSampling() {
    ...
    if (tDelay < (m_tBaseline + m_tBaseline / 280)) {
        m_tBaseline = m_tBaseline + (tDelay - m_tBaseline) / 2;
    }
#ifdef CAPTOUCH_DEBUG
...
CapTouch::Event CapTouch::touchEventCheck() {
    ...
    if (m_tReading > (m_tBaseline + m_tBaseline / 300)) {
        touchSense = HIGH;
    } else {
        touchSense = LOW;
    }
